...
use crate::{CandlesBidAsk, CandleTypeCache, CandleType, CandleModel};
...
impl CandlesInstrumentsCache {
    ...
    pub async fn handle_new_bid_ask(&self, prices: Vec<CandlesBidAsk>){
        self.handle_new_bid_ask_internal(true, &prices).await;
        self.handle_new_bid_ask_internal(false, &prices).await;
    }

    async fn handle_new_bid_ask_internal(&self, is_bid: bool, prices: &Vec<CandlesBidAsk>){
    ...
    pub async fn init(&self, instument_id: String, is_bid: bool, candle_type: CandleType, candle: CandleModel){
    ...
    pub async fn get_by_date_range(&self, instument_id: String, candle_type: CandleType, is_bid: bool, start_date: u64, end_date: u64) -> Vec<CandleModel>{
        let mut target_cache = match is_bid {
            true => self.bid_candles.write().await,
            false => self.ask_candles.write().await,
        };
        ...
        match instrument_cache {
            Some(cache) => {
                cache.get_by_date_range(candle_type, start_date, end_date)
            },
            None => {
                vec![]
            },
...
mod tests {
    ...
    async fn test_sinle_quote() {
        ...
        cache.handle_new_bid_ask(vec![bid_ask]).await;

        let result_bid_minute = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Minute, true, 1660559404, 2660559404).await;
        let result_ask_minute = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Minute, false, 1660559404, 2660559404).await;

        let result_bid_hour = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Hour, true, 1660559404, 2660559404).await;
        let result_ask_hour = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Hour, false, 1660559404, 2660559404).await;

        let result_bid_day = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Day, true, 1660559404, 2660559404).await;
        let result_ask_day = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Day, false, 1660559404, 2660559404).await;

        let result_bid_mount = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Month, true, 1660559404, 2660559404).await;
        let result_ask_mount = cache.get_by_date_range(instument.clone(), crate::models::CandleType::Month, false, 1660559404, 2660559404).await;
